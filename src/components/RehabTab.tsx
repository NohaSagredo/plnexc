...
import { TRANSLATIONS, translateEngineText } from '../utils/translations';
...
export default function RehabTab({ activeInjury, setActiveInjury, language }: RehabTabProps) {
  ...
  const handleAnalyzeScreening = () => {
   ...
   const handleClearInjury = () => {
      if (window.confirm(t.rehabRecoveredConfirm || '¿Te has recuperado por completo de tu molestia? ¡Felicidades! Se reestablecerá tu entrenamiento habitual.')) {
      ...
      <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div className="glass-panel" style={{ padding: '24px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <h2 style={{ fontSize: '1.5rem', fontWeight: 800 }}>{t.rehabTitle || 'PLNEXC Rehab Assistant'}</h2>
              <p style={{ color: 'hsl(var(--muted))', fontSize: '0.875rem', marginTop: '2px' }}>
                {t.rehabSubtitleDesc || 'Autoevaluación física y rehabilitación de lesiones adaptada para PLNEXC (según Aaron Horschig)'}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="glass-panel glass-panel-glow" style={{ padding: '24px', borderLeft: '4px solid hsl(var(--warning))' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
              <div>
                <span className="badge badge-warning" style={{ marginBottom: '8px' }}>
                  {(t.rehabActiveProtocol || 'Protocolo Activo: {name}').replace('{name}', translateEngineText(MILO_REHAB_PROTOCOLS[activeInjury.joint]?.displayName || '', language))}
                </span>
                <h3 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#ffffff' }}>
                  {t.rehabWeaknessDetected || 'Eslabón Débil Detectado:'} <span style={{ color: 'hsl(var(--warning))' }}>{translateEngineText(activeInjury.weakness, language)}</span>
                </h3>
                <p style={{ color: 'hsl(var(--muted))', fontSize: '0.875rem', marginTop: '4px', maxWidth: '600px' }}>
                  {(() => {
                    const phaseNameStr = translateEngineText(MILO_REHAB_PROTOCOLS[activeInjury.joint]?.phases[activeInjury.phase]?.name || '', language);
                    const desc = t.rehabCurrentPhaseDesc || 'Estás actualmente en la {phaseName}. El algoritmo ha modificado de forma adaptativa tu rutina de entrenamiento y ha integrado este calentamiento obligatorio.';
                    const parts = desc.split('{phaseName}');
                    return (
                      <>
                        {parts[0]}<strong>{phaseNameStr}</strong>{parts[1]}
                      </>
                    );
                  })()}
                </p>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                {activeInjury.phase < 3 && (
                  <button className="btn btn-secondary" onClick={handleProgressPhase} style={{ color: 'hsl(var(--success))' }}>
                    {t.rehabNextPhaseBtn || 'Avanzar de Fase'}
                  </button>
                )}
                <button className="btn btn-danger" onClick={handleClearInjury}>
                  {t.rehabRecoveredBtn || 'Marcar como Recuperado'}
                </button>
              </div>
            </div>
            ...
            <div style={{ borderTop: '1px solid hsl(var(--border))', marginTop: '20px', paddingTop: '20px' }}>
              <h4 style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '1.1rem', marginBottom: '12px' }}>
                ...
                {t.rehabCorrectiveRoutineTitle || 'Rutina Correctiva Recomendada'}
              </h4>
                ...
                {MILO_REHAB_PROTOCOLS[activeInjury.joint]?.phases[activeInjury.phase]?.exercises.map((ex: CorrectiveExercise) => (
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px', marginBottom: '6px' }}>
                      <strong style={{ color: 'hsl(var(--warning))' }}>{translateEngineText(ex.name, language)}</strong>
                      <span className="badge badge-primary">{ex.sets} {t.series || 'series'} x {translateEngineText(ex.reps, language)}</span>
                    </div>
                    <p style={{ fontSize: '0.875rem', color: '#e5e7eb', marginBottom: ex.instructions ? '12px' : '0' }}>
                      {translateEngineText(ex.description, language)}
                    </p>
                        ...
                        {(() => {
                          ...
                          return (
                            <>
                              ...
                              >
                                <BookOpen size={12} />
                                <span>{isExpanded ? (t.rehabHideGuideBtn || 'Ocultar guía de técnica') : (t.rehabShowGuideBtn || 'Ver guía de técnica paso a paso')}</span>
                              </button>
                                ...
                                <div style={{ 
                                  ...
                                  {ex.image && (
                                    <div style={{ width: '100%', maxHeight: '160px', overflow: 'hidden', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.1)' }}>
                                      <img 
                                        src={ex.image} 
                                        alt={translateEngineText(ex.name, language)} 
                                        loading="lazy" 
                                  ...
                                  <ol style={{ paddingLeft: '16px', margin: 0, fontSize: '0.8rem', color: '#d1d5db', display: 'flex', flexDirection: 'column', gap: '6px', lineHeight: '1.4' }}>
                                    {ex.instructions.map((step, sIdx) => (
                                      <li key={sIdx}>{translateEngineText(step, language)}</li>
                                    ))}
                                  </ol>
                                </div>
                              )}
        ...
        {!showScreening && !activeInjury ? (
          <div className="glass-panel" style={{ padding: '24px' }}>
            <h3 style={{ fontSize: '1.25rem', marginBottom: '8px' }}>{t.rehabFeelDiscomfortTitle || '¿Sientes molestias en algún levantamiento?'}</h3>
            <p style={{ color: 'hsl(var(--muted))', fontSize: '0.875rem', marginBottom: '20px' }}>
              {t.rehabFeelDiscomfortDesc || 'Selecciona la articulación afectada para iniciar tu sesión de evaluación física y obtener un diagnóstico clínico.'}
            </p>
  
            <div className="grid-cols-3">
               {Object.values(MILO_REHAB_PROTOCOLS).map((jointProto) => {
                 const isSelected = selectedJoint === jointProto.joint;
                 const jointNameTrans = translateEngineText(jointProto.displayName, language);
                 return (
                   <div 
                   ...
                   >
                     ...
                     <div>
                       <strong style={{ fontSize: '1.1rem', color: isSelected ? '#ffffff' : 'rgba(255,255,255,0.9)' }}>{jointNameTrans}</strong>
                       <p style={{ color: 'hsl(var(--muted))', fontSize: '0.75rem', marginTop: '4px' }}>
                         {(t.rehabMobilityScreensCount || '{count} Pruebas de Movilidad').replace('{count}', jointProto.screens.length.toString())}
                       </p>
                     </div>
                     <button 
                     ...
                     >
                       {t.rehabAssessJointBtn || 'Evaluar Articulación'}
                     </button>
                   </div>
                 );
               })}
        ...
        {showScreening && (
          <div className="glass-panel" style={{ padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid hsl(var(--border))', paddingBottom: '16px', marginBottom: '20px' }}>
              <div>
                <span className="badge badge-primary">{t.rehabAssessInProgress || 'Evaluación en Progreso'}</span>
                <h3 style={{ fontSize: '1.4rem', fontWeight: 800, marginTop: '4px' }}>
                  {(t.rehabDiagnosisOf || 'Diagnóstico de: {name}').replace('{name}', translateEngineText(protocol.displayName, language))}
                </h3>
              </div>
             ...
             <div style={{ marginBottom: '24px' }}>
               <label style={{ fontSize: '0.9rem', color: 'hsl(var(--muted))', fontWeight: 600 }}>
                 {t.rehabPainLevelQuestion || '¿Qué nivel de dolor o molestia experimentas habitualmente en esta articulación? (1-10)'}
               </label>
             ...
             <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', color: 'hsl(var(--muted))', marginTop: '4px' }}>
               <span>{t.rehabPainScaleMild || 'Leve rigidez (1)'}</span>
               <span>{t.rehabPainScaleModerate || 'Moderado (5)'}</span>
               <span>{t.rehabPainScaleSevere || 'Dolor Agudo / Impedimento (10)'}</span>
             </div>
             </div>
           ...
           <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
             <h4 style={{ fontSize: '1.1rem', borderBottom: '1px solid hsl(var(--border))', paddingBottom: '8px' }}>
               {t.rehabMandatoryTestsTitle || 'Pruebas Físicas Obligatorias'}
             </h4>
             ...
             {protocol.screens.map((screen: ScreenTest, idx) => (
               ...
               >
                 <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                   ...
                   <strong>{(t.rehabTestTitle || 'Prueba {index}: {name}').replace('{index}', (idx+1).toString()).replace('{name}', translateEngineText(screen.name, language))}</strong>
                 </div>
                 
                 <p style={{ fontSize: '0.875rem', color: '#e5e7eb', marginBottom: '10px' }}>
                   <strong>{t.rehabTestHowTo || 'Cómo hacerlo:'}</strong> {translateEngineText(screen.instruction, language)}
                 </p>
                 
                 <div style={{ background: 'rgba(0,0,0,0.2)', padding: '10px', borderRadius: '6px', fontSize: '0.8rem', color: 'hsl(var(--muted))', marginBottom: '12px' }}>
                   <strong>{t.rehabTestFailureCriteria || 'Criterio de Fallo:'}</strong> {translateEngineText(screen.criteria, language)}
                 </div>
                 ...
                 <div style={{ display: 'flex', gap: '12px' }}>
                   ...
                   >
                     {t.rehabTestFailBtn || '⚠️ Siento Dolor / Restricción (Falla)'}
                   </button>
                   <button 
                   ...
                   >
                     <CheckCircle2 size={16} /> {t.rehabTestPassBtn || 'Pasa Correctamente'}
                   </button>
                 </div>
               </div>
             ))}
           ...
           >
             {t.rehabAnalyzeBtn || 'Analizar Diagnóstico e Iniciar Rehabilitación'}
           </button>
         </div>
       )}
       ...
       <div className="glass-panel" style={{ padding: '20px', background: 'rgba(0, 242, 254, 0.02)', display: 'flex', gap: '12px', alignItems: 'center' }}>
         ...
         <span style={{ fontSize: '0.85rem', color: 'hsl(var(--muted))' }}>
           {(() => {
             const phil = t.rehabPhilosophyFull || 'Filosofía PLNEXC Rehab: "Nunca permitas que la lesión te detenga". Si un ejercicio te causa dolor en la escala de 3 o superior, utiliza el botón de sustituto de la rutina. La sobrecarga progresiva solo funciona si no hay dolor de por medio.';
             const parts = phil.split(':');
             if (parts.length > 1) {
